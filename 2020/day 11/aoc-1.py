...
def getAdjacentOccupiedSpaces(i, j, currentSeatplan):
    occupied = 0
    # above
    if i > 0 and currentSeatplan[i-1][j] == '#':
        occupied += 1

    # below
    if i < len(currentSeatplan) - 1 and currentSeatplan[i+1][j] == '#':
        occupied += 1

    # left
    if j > 0 and currentSeatplan[i][j-1] == '#':
        occupied += 1

    # right
    if j < len(currentSeatplan[i]) - 1 and currentSeatplan[i][j+1] == '#':
        occupied += 1

    # upper-left
    if i > 0 and j > 0 and currentSeatplan[i-1][j-1] == '#':
        occupied += 1

    # upper-right
    if i > 0 and j < len(currentSeatplan[i]) - 1 and currentSeatplan[i-1][j+1] == '#':
        occupied += 1

    # bottom-left
    if i < len(currentSeatplan) - 1 and j > 0 and currentSeatplan[i+1][j-1] == '#':
        occupied += 1

    # bottom-right
    if i < len(currentSeatplan) - 1 and j < len(currentSeatplan[i]) - 1 and currentSeatplan[i+1][j+1] == '#':
        occupied += 1
...
def shouldBecomeFree(i, j, currentSeatplan):
    return getAdjacentOccupiedSpaces(i, j, currentSeatplan) >= 4
