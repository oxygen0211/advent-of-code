...
def getChildCount(colorChildren, color):
    children = 1
    print('---------{}--------------'.format(color))
    for child in colorChildren[color]:
        col = child['color']
        amount = child['amount']
        #print('child: {} * {}'.format(col, amount))
        childCount = getChildCount(colorChildren, col)
        if childCount < 1:
            childCount = 1
        print('{} * {} {}'.format(amount, childCount, col))
        children += amount * childCount
        print('children: {}'.format(children))
    return children

colorChildren = {}
for rule in input:
    color = rule[:rule.index('bags')].strip()

    containments = rule[rule.index('contain') + 7:].strip()

    colorChildren[color] = []
    for containment in containments.split(','):
        if containment.startswith('no'):
            continue

        containment = containment.strip()

        number = containment[:1]
        col = containment[1:containment.index('bag')].strip()

        colorChildren[color].append({'color': col, 'amount': int(number)})

#print(colorChildren)
goldChildren = getChildCount(colorChildren, 'shiny gold')
print(goldChildren - 1)
