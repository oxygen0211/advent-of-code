...
def getParentCount(colorParents, color):
    if not color in colorParents.keys():
        return []
    parents = []
    for parent in colorParents[color]:
        parents.append(parent)
        colors = getParentCount(colorParents, parent)
        for col in colors:
...
for rule in input:
    ...
    for containment in containments.split(','):
        ...
        parents = []
        if col in colorParents.keys():
            parents = colorParents[col]

        if not color in parents:
            parents.append(color)

        colorParents[col] = parents

print(colorParents)
goldParents = getParentCount(colorParents, 'shiny gold')
