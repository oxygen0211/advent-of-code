...
for entry in input:
    ...
    byrValid = 'byr' in fields and int(fields['byr']) >= 1920 and int(fields['byr']) <= 2002
    iyrValid = 'iyr' in fields and int(fields['iyr']) >= 2010 and int(fields['iyr']) <= 2020
    eyrValid = 'eyr' in fields and int(fields['eyr']) >= 2020 and int(fields['eyr']) <= 2030
    hgtValid = False
    if 'hgt' in fields:
        value = fields['hgt'][:len(fields['hgt'])-2]
        unit = fields['hgt'][len(fields['hgt'])-2:]

        if unit == 'cm':
            hgtValid = int(value) >=150 and int(value) <= 193

        if unit == 'in':
            hgtValid = int(value) >=59 and int(value) <= 76

    hclValid = 'hcl' in fields and len(fields['hcl']) == 7 and hclRegex.match(fields['hcl'])
    eclValid = 'ecl' in fields and fields['ecl'] in allowedEyeColors
    pidValid = 'pid' in fields and len(fields['pid']) == 9

    if byrValid and iyrValid and eyrValid and hgtValid and hclValid and eclValid and pidValid:
        validEntrys += 1

    else:
        print('invalid. Fields: {}. byrValid: {} iyrValid: {} eyrValid: {} hgtValid: {} hclValid: {} eclValid: {} pidValid: {}'.format(fields, byrValid, iyrValid, eyrValid, hgtValid, hclValid, eclValid, pidValid))

print('Valid entries: {}'.format(validEntrys))
